    ...
    public Map<String, Object> isPlayerInCooldown(UUID playerId) {
        ...
        try (Connection connection = getConnection();
             PreparedStatement statement = connection.prepareStatement(
                     "SELECT death_data, revival_method FROM `" + tableName +
                             "` WHERE uuid = ? AND server_name = ? ORDER BY update_date DESC LIMIT 1")) {
    ...
    public void insertPlayerDeathData(final UUID playerId, final String deathData, final String revivalMethod) {
        new BukkitRunnable() {
            ...
            public void run() {
                try (Connection connection = getConnection();
                     PreparedStatement statement = connection.prepareStatement(
                             "INSERT INTO `" + tableName + "` (uuid, server_name, death_data, revival_method) VALUES (?, ?, ?, ?)")) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                try (Connection connection = getConnection()) {
                    ...
                    String deathDataJson = deathData.toString();
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                try (Connection connection = getConnection();
                     PreparedStatement statement = connection.prepareStatement(
                             "UPDATE `" + tableName +
                                     "` SET revival_method = ? WHERE uuid = ? AND server_name = ? " +
                                     "ORDER BY update_date DESC LIMIT 1")) {
